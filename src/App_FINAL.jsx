...
import ScenerySlideshow from "./ScenerySlideshow";
...
useEffect(() => {
  ...
  return;
    ...
    if (!String(nextRow.video_id || "").startsWith("usb:")) {
      startSongTransition(5000);
    }
  ...
  }, [normalizeQueuePositions, startSongTransition]);
   useEffect(() => {
...
setTransitionMediaReady(false);
  ...
  return (
    <main className={`tv-shell ${showTextBanner ? "has-announcement" : ""}`}>
      <TvHeader
        status={status}
        onOpenQr={openQrPopup}
      />

{showTextBanner && (
  <AnnouncementBanner message={textBannerMessage} />
)}
...
{!song && (
  ...
  <div className="popup-overlay">
    ...
    />
      ...
      {showQrPopup && (
  <QrPopup onClose={closeQrPopup} />
)}
      {sceneryShow && (
  <ScenerySlideshow imageSrc={sceneryImages[sceneryIndex]} />
)}
   </main>
  );
}
